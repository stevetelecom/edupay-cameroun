    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">
        <div>
            <div style="font-size:18px;font-weight:700;">
                Bonjour, {{ Str::of(Auth::user()->name)->explode(' ')->first() }}
            </div>
        ...
        <div class="g2" style="margin-bottom:18px;">
            @foreach($apprenants as $apprenant)
                @php
                    ...
                    $premierFraisImpaye = $apprenant->frais->firstWhere('statut', '!=', 'regle');
                @endphp
                <div class="epcard" style="border-left:3px solid {{ match($statut) {
                    ...
                    @if($resteApprenant > 0)
                        ...
                        @if($premierFraisImpaye)
                            <a href="{{ route('payeur.paiement.show', $premierFraisImpaye) }}"
                               style="{{ $statut === 'impaye' ? 'background:var(--ep-red);color:#fff;border:none;padding:8px;border-radius:var(--radius-md);font-size:12px;cursor:pointer;width:100%;text-align:center;text-decoration:none;display:block;' : '' }}"
                               class="{{ $statut === 'impaye' ? '' : 'btn-p' }}"
                               @if($statut !== 'impaye') style="font-size:12px;padding:8px;" @endif>
                                {{ $statut === 'impaye' ? 'Payer maintenant →' : 'Continuer le paiement →' }}
                            </a>
                        @endif
                    @else
