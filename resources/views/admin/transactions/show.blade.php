...
<div class="space-y-4">
    ...
    <div class="grid grid-cols-2 gap-3">
        <div style="background:#f9fafb;border-radius:8px;padding:12px;">
            <div style="font-size:11px;color:#999;margin-bottom:4px;">Ecole</div>
            <div style="font-size:13px;font-weight:600;color:#111;">
                {{ $paiement->fraisApprenant?->categorieFrais?->etablissement?->nom ?? "—" }}
            </div>
        </div>
        <div style="background:#f9fafb;border-radius:8px;padding:12px;">
            <div style="font-size:11px;color:#999;margin-bottom:4px;">Apprenant</div>
            <div style="font-size:13px;font-weight:600;color:#111;">
                {{ $paiement->apprenant ? $paiement->apprenant->nom." ".$paiement->apprenant->prenom : "—" }}
            </div>
        </div>
        <div style="background:#f9fafb;border-radius:8px;padding:12px;">
            <div style="font-size:11px;color:#999;margin-bottom:4px;">Montant</div>
            <div style="font-size:18px;font-weight:700;color:#0D9E75;">
                {{ number_format($paiement->montant, 0, ",", " ") }} FCFA
            </div>
        </div>
        <div style="background:#f9fafb;border-radius:8px;padding:12px;">
            <div style="font-size:11px;color:#999;margin-bottom:4px;">Operateur</div>
            <div style="font-size:13px;font-weight:600;color:#111;">
                {{ $paiement->operateur ?? "—" }}
            </div>
        </div>
        <div style="background:#f9fafb;border-radius:8px;padding:12px;">
            <div style="font-size:11px;color:#999;margin-bottom:4px;">Telephone</div>
            <div style="font-size:13px;font-weight:600;color:#111;">
    ...
    <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:10px 14px;">
        <div style="font-size:11px;color:#166534;margin-bottom:2px;">Token AangaraaPay</div>
