...
@section('title', __('messages.reclamations'))
...
<div id="modal-detail-rec" class="ep-modal-overlay">
  <div class="ep-modal ep-modal-md">
    <div class="ep-modal-head">
      <h3>{{ __('admin.detail_reclamation') }}</h3>
...
<div id="modal-repondre" class="ep-modal-overlay">
  <div class="ep-modal ep-modal-md">
    <div class="ep-modal-head">
      <h3>{{ __('admin.repondre_reclamation') }}</h3>
      <button class="ep-modal-close" onclick="epModal.close('modal-repondre')">x</button>
    </div>
    <div class="ep-modal-body">
      <div style="margin-bottom:12px;">
        <div style="font-size:12px;color:#888;margin-bottom:2px;">{{ __('admin.ticket_col') }}</div>
        <div style="font-size:13px;font-weight:600;color:#111;" id="repondre-ticket"></div>
      </div>
      <form id="form-repondre" method="POST">
        @csrf @method('PATCH')
        <div style="margin-bottom:14px;">
          <label style="font-size:12px;font-weight:500;color:#555;display:block;margin-bottom:6px;">{{ __('admin.reponse_etoile') }}</label>
          <textarea name="reponse_admin" rows="5" required
                    placeholder="{{ __('admin.ph_reponse') }}"
        ...
        <div style="margin-bottom:16px;">
          <label style="font-size:12px;font-weight:500;color:#555;display:block;margin-bottom:6px;">{{ __('admin.nouveau_statut_etoile') }}</label>
          <select name="statut" required
                  style="width:100%;padding:10px 12px;font-size:13px;border:1px solid #ddd;border-radius:8px;outline:none;">
            <option value="en_cours">{{ __('admin.en_cours_traitement') }}</option>
            <option value="resolu">{{ __('admin.resolu') }}</option>
            <option value="rejete">{{ __('admin.rejete') }}</option>
          </select>
        </div>
...
<div class="bg-white border border-gray-200 rounded-xl p-4 mb-4">
  <form method="GET" action="{{ route('admin.reclamations.index') }}" class="flex items-center gap-3 flex-wrap">
    <input type="text" name="search" value="{{ request('search') }}"
           placeholder="{{ __('admin.ph_recherche_ticket') }}"
    ...
    <select name="statut" class="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:border-[#0D9E75]">
      ...
      <option value="ouvert"   {{ request('statut')==='ouvert'   ? 'selected' : '' }}>{{ __('admin.ouvert') }}</option>
      <option value="en_cours" {{ request('statut')==='en_cours' ? 'selected' : '' }}>{{ __('admin.en_cours') }}</option>
      <option value="resolu"   {{ request('statut')==='resolu'   ? 'selected' : '' }}>{{ __('admin.resolu') }}</option>
      <option value="rejete"   {{ request('statut')==='rejete'   ? 'selected' : '' }}>{{ __('admin.rejete') }}</option>
    </select>
...
<div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
  <div class="responsive-admin-table-container">
    ...
    <thead class="bg-gray-50 border-b border-gray-200">
      <tr>
        <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">{{ __('admin.ticket_col') }}</th>
        <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">{{ __('admin.client_col') }}</th>
    ...
    <tbody class="divide-y divide-gray-100">
        ...
        <td class="px-4 py-3">
          <div class="flex items-center justify-center gap-1.5">
            ...
            <button onclick="ouvrirDetailRec({{ $r->id }})"
                    class="w-7 h-7 flex items-center justify-center rounded-lg bg-[#E0F5EE] hover:bg-[#c4eadb] text-[#0D9E75] transition-colors" title="{{ __('admin.detail_reclamation') }}">
            ...
            <button onclick="ouvrirRepondre({{ $r->id }}, {{ json_encode($r->numero_ticket) }})"
                    class="w-7 h-7 flex items-center justify-center rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 transition-colors" title="{{ __('admin.repondre_reclamation') }}">
      ...
      @empty
      <tr>
        <td colspan="6" class="px-4 py-10 text-center text-sm text-gray-400">{{ __('admin.aucune_reclamation') }}</td>
      </tr>
      @endforelse
    </tbody>
    </table>
