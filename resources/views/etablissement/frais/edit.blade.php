...
@section('title', __('etablissement.modifier_frais_titre', ['nom' => $frais->nom]))
...
<div style="display:flex;align-items:center;gap:8px;margin-bottom:20px;font-size:13px;">
    ...
    <span style="font-weight:600;">{{ __('etablissement.modifier_frais_bread', ['nom' => $frais->nom]) }}</span>
</div>
...
<div class="epcard" style="margin-bottom:14px;">
    <div class="seclbl" style="margin-top:0;">{{ __('etablissement.categorie_bread') }}</div>
    ...
    <div class="lbl">{{ __('etablissement.nb_tranches_max_label') }}</div>
    <div style="display:flex;gap:8px;margin-bottom:12px;">
        ...
        <label style="flex:1;padding:10px;border:2px solid {{ old('nb_tranches_max',$frais->nb_tranches_max)==$n?'var(--ep-teal)':'#ddd' }};
            ...
            <div style="font-size:10px;color:#888;">{{ $n>1 ? __('etablissement.tranches_unit') : __('etablissement.tranche_unit') }}</div>
        </label>
        @endforeach
    </div>
    ...
    <label style="display:flex;align-items:center;gap:8px;font-size:13px;margin-bottom:12px;cursor:pointer;">
        ...
        {{ __('etablissement.categorie_active') }}
    </label>
...
<button type="submit" class="btn-p" style="width:100%;">{{ __('etablissement.modifier_enregistrer') }}</button>
...
<div>
<div class="epcard">
    <div class="seclbl" style="margin-top:0;">{{ __('etablissement.echeances_config') }}</div>
    ...
    <div style="color:#aaa;font-size:12px;text-align:center;padding:12px;">
        {{ __('etablissement.aucune_echeance_msg') }}
    </div>
    @endforelse
</div>
</div>
