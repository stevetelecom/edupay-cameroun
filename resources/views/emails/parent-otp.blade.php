...
<head>
    ...
    <style>
        ...
        .code { font-size:44px; font-weight:800; letter-spacing:10px; color:#0B2545; font-family:'Courier New',monospace; }
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="header-sub">{{ __('auth.em_otp_header_sub') }}</div>
        </div>
        <div class="body">
            ...
            <div class="code-box">
                ...
                <div class="expires">{{ __('auth.em_otp_code_expire') }}</div>
            </div>

            <div style="background:#f8f9fa;border-radius:8px;padding:14px 16px;margin-bottom:20px;">
                <div class="info-row">
                    <span class="info-label">{{ __('auth.em_label_compte') }}</span>
                    <span class="info-val">{{ $user->email ?? $user->telephone }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">{{ __('auth.em_label_date') }}</span>
                    <span class="info-val">{{ now()->format('d/m/Y à H:i') }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">URL</span>
                    <span class="info-val">{{ config('app.url') }}/connexion</span>
                </div>
            </div>

            <div class="warning">
                <p>{!! __('auth.em_otp_securite_label') !!}
                {{ __('auth.em_otp_securite_texte') }}
                {{ __('auth.em_otp_code_confidentiel') }}</p>
            </div>
        </div>
        <div class="footer">
